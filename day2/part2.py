...
def on_forward(magnitude: int)->Callable[[SubmarineState], SubmarineState]:
    ...
    def handler(curr_state: SubmarineState):
        ...
        new_depth = curr_state.depth + curr_state.aim*magnitude
        new_aim = curr_state.aim
        return SubmarineState(new_horizontal_pos, new_depth, new_aim)
    return handler
...
def on_down(magnitude: int)->Callable[[SubmarineState], SubmarineState]:
    ...
    def handler(curr_state: SubmarineState):
        ...
        new_aim = curr_state.aim+magnitude
        return SubmarineState(new_horizontal_pos, new_depth, new_aim)
    return handler
...
class Submarine:
    ...
    def __init__(self) -> None:
        self.submarine_state = SubmarineState(0, 0, 0)
    ...
    def aim(self):
        """Returns the current aim of the submarine."""
        return self.submarine_state.aim
