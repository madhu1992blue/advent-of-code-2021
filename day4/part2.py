...
def last_winner_and_last_num(
        boards: Iterable[Board], numbers_called: Iterable[int]
    )->Optional[Tuple[Board, int]]:
    """Determines the last winner and last num that created the winner."""
    for num in numbers_called:
        new_boards = [board.mark_num(num) for board in boards]
        if all([board.has_won() for board in new_boards]):
            return new_boards[0], num
        boards = [board for board in new_boards if not board.has_won()]
    return None
...
with open(Path(__file__).parent / "day4_input") as f:
    ...
    last_winner_last_num = last_winner_and_last_num(ip_boards, ip_numbers_called)
    if last_winner_last_num is not None:
        last_winner, last_num = last_winner_last_num
        print(sum(last_winner.unmarked())*last_num)
